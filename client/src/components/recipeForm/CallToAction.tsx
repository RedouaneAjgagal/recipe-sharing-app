...
const CallToAction = (props: React.PropsWithoutRef<Props>) => {
  ...
  return (
    <div className={`py-4 grid gap-2 ${props.for === "newRecipe" ? "grid-cols-2" : "grid-cols-3 fixed bottom-0 left-0 w-full bg-white px-2"}`}>
      ...
      <button disabled={props.isSubmitting} className='bg-amber-700 col-span-2 w-full text-white py-2 rounded font-medium tracking-wide flex justify-center items-center'>{props.isSubmitting ?
